...
class GenerationOrchestrator:
    ...
    async def research(self, query: str, template: str, user_id: UUID) -> dict:
        ...
        payload = payload_from_research(query, chunks)
        template_key = payload.get("eventData", {}).get("story", {}).get("templateType") or "universal"
    ...
    async def create(self, content: str, template: str, user_id: UUID) -> dict:
        ...
        # parsed["template"] do LLM tự phân loại
        parsed = parse_creator_content(content)
        template_key = parsed.get("template") or "universal"
        coverage = check_story_event_coverage(parsed, template_key)
        if coverage["missing"]:
            payload = story_event_shell(parsed["title"], template_key, "custom_content", "creator", parsed["summary"])
        ...
        payload = payload_from_creator(parsed, coverage)
        return await self.page_manager.save_story_event_page(
            user_id=user_id,
            title=payload["title"],
            flow_type="custom_content",
    ...
    async def confirm_missing(self, page_id: UUID, user_id: UUID) -> dict:
        ...
        template_key = parsed.get("template") or context["template_key"] or "universal"
